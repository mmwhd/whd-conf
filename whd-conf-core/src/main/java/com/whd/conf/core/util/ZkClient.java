package com.whd.conf.core.util;
...
public class ZkClient {
	...
	public ZooKeeper getClient(){
				...
				if (INSTANCE_INIT_LOCK.tryLock(2, TimeUnit.SECONDS)) {
					if (zooKeeper==null) {		// 二次校验，防止并发创建client
						...
						try {
							...
							zooKeeper = newZk;
							logger.info(">>>>>>>>>> conf, ZkClient init success.");
						} catch (Exception e) {
		...
		if (zooKeeper == null) {
			throw new ConfException("ZkClient.zooKeeper is null.");
		}
		return zooKeeper;
	}
	...
	public String getPathData(String path){
		try {
			...
			if (stat != null) {
				...
				logger.info(">>>>>>>>>> conf, path[{}] not found.", path);
			}
			return znodeValue;
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
