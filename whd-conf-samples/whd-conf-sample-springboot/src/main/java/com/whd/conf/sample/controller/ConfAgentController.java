...
/**
 * Conf Agent, support multilingual applications.
 ...
 * 		3、高性能：得益于 Conf 底层实现的 Local Cache，因此该 Agent 服务性能非常高；单机可承担大量配置请求；
 * 		4、集群部署：Conf Agent 支持集群部署，提高配置服务的可用性；
 *
 * @author hayden 2018-05-29 20:43:17
 */
@Controller
@RequestMapping("/confagent")
public class ConfAgentController {
	private static Logger logger = LoggerFactory.getLogger(ConfAgentController.class);
	...
	public Object index(String accessToken,
		...
		if (Boolean.valueOf(async)) {
			...
			for (String confKey: confKeyList) {
				String confValue = ConfClient.get(confKey.trim(), null);
				if (confValue!=null) {
	...
	public void initAgentListener(){
		ConfClient.addListener(null, new ConfListener() {
			@Override
